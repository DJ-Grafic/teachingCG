...
namespace GMath
{
  public static partial class Gfx
  {
    ...
    public static float3 randomInCylinder()
    {
      ...
      float t = random();
      return float3((float)(Math.Cos(theta)), (float)(Math.Sin(theta)), t);
    }
    ...
    public static float3 randomInHyperboloid()
    {
      float theta = random() * two_pi;
      float v = random();

      return float3(
        (float)(Math.Sinh(v) * Math.Cos(theta)),
        (float)(Math.Cosh(v) * Math.Sin(theta)),
        (float)(Math.Sinh(v))
      );
    }
    #endregion
